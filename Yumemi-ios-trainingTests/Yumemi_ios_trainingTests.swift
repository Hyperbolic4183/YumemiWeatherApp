...
class Yumemi_ios_trainingTests: XCTestCase {
    ...
    func test_天気予報がsunnyだったときに画面に晴れ画像が表示される() {
        ...
        viewController.showIndicator(processing: viewController.weatherModel.fetchYumemiWeather) { _ in
            DispatchQueue.main.async {
                viewController.updateView(.success(sunnyWeatherInformation))
    ...
    func test_天気予報がcloudyだったときに画面に曇り画像が表示される() {
        ...
        viewController.showIndicator(processing: viewController.weatherModel.fetchYumemiWeather) { _ in
            DispatchQueue.main.async {
                viewController.updateView(.success(cloudyWeatherInformation))
    ...
    func test_天気予報がrainyだったときに画面に雨画像が表示される() {
        ...
        viewController.showIndicator(processing: viewController.weatherModel.fetchYumemiWeather) { _ in
            DispatchQueue.main.async {
                viewController.updateView(.success(rainyWeatherInformation))
    ...
    func test_最高気温がUILabelに反映される() {
        ...
        viewController.showIndicator(processing: viewController.weatherModel.fetchYumemiWeather) { _ in
            DispatchQueue.main.async {
                viewController.updateView(.success(maxTemperatureWeatherInformation))
                XCTAssertEqual(view.maxTemperatureLabel.text, testingMaxTemperature)
                expectation?.fulfill()
            }
        }
        waitForExpectations(timeout: 3, handler: nil)
    ...
    func test_最低気温がUILabelに反映される() {
        ...
        viewController.showIndicator(processing: viewController.weatherModel.fetchYumemiWeather) { _ in
            DispatchQueue.main.async {
                viewController.updateView(.success(minTemperatureWeatherInformation))
                XCTAssertEqual(view.minTemperatureLabel.text, testingMinTemperature)
                expectation?.fulfill()
            }
        }
        waitForExpectations(timeout: 3, handler: nil)
